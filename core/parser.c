...
ulang_result_t ulang_parser_load(ulang_parser_t *parser, ulang_tokens_t tokens) {
  ...
  nob_da_append(&parser->types, intType);
...
ulang_result_t ulang_parser_parse_toplevel(ulang_parser_t *parser, ulang_ast_toplevel_t *toplevel) {
  ...
  case ULANG_TOKEN_ID: {
    ...
    if (ulang_parser_is_type(parser, token.value.as.string, &type) || (func_only = nob_sv_eq(token.value.as.string, SV("func")))) {
      ulang_token_t tok = {0};
      ...
      if (tok.kind == '(') { // Function def
        *toplevel = (ulang_ast_toplevel_t){
        ...
        ulang_result_t result = ulang_parser_parse_funcdef(parser, &toplevel->as.funcdef);
        if (result.kind != ULANG_SUCCESS) return result;
      } else if (!func_only && (tok.kind == ';' || tok.kind == '=')) { // Var def
        *toplevel = (ulang_ast_toplevel_t){
          .kind = ULANG_AST_TOPLEVEL_VARDEF
        };
        ulang_result_t result = ulang_parser_parse_vardef(parser, &toplevel->as.vardef);
        if (result.kind != ULANG_SUCCESS) return result;
      } else {
        return (ulang_result_t){
          .kind = ULANG_PACK_RESULT_KIND(ULANG_PARSER_ERROR, ULANG_UNEXPECTED_TOKEN),
...
ulang_result_t ulang_parser_parse_expr(ulang_parser_t *parser, ulang_ast_expr_t *expr) {
  ulang_token_t token = {0};
  if (!ulang_parser_peek(parser, 0, &token) || !ulang_parser_advance(parser, NULL)) return (ulang_result_t){
    .kind = ULANG_PACK_RESULT_KIND(ULANG_PARSER_ERROR, ULANG_UNEXPECTED_EOF)
  };
...
ulang_result_t ulang_parser_parse_vardef(ulang_parser_t *parser, ulang_ast_stmt_vardef_t *vardef) {
  ...
  ulang_result_t result = {0};
  if ((result = ulang_parser_eat(parser, ULANG_TOKEN_ID, &type_tok)).kind != ULANG_SUCCESS) return result;
  if (!ulang_parser_is_type(parser, type_tok.value.as.string, &type)) return (ulang_result_t){
